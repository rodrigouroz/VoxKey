...
**Model Override**:
A library of four model cards, reached from onboarding, Settings, and the menu bar. Users can download multiple models independently of activation; downloading leaves the current model available, and exactly one model is active. An explicit setting in Models and Languages replaces the Default Model with full Distil-Whisper v3 (1.51 GB), compact Whisper v3 Turbo (627 MB), or full Whisper v3 Turbo (1.62 GB). All choices use WhisperKit and Core ML. Distil supports English only; Turbo also supports Spanish and the languages listed in the chooser. Language selection includes automatic detection for Turbo. Activation persists the model and language only after successful preparation; failure preserves the previous working configuration. Preparation pauses dictation, and an active dictation prevents model changes. The English grammar corrector is paused for non-English and Automatic sessions. Distil-v3.5 conversion and multilingual grammar correction are research candidates, not shipping model choices. Model size is storage, not a measured memory or latency guarantee.
